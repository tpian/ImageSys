...
parser.add_argument('--label_txt', type=str, default='classifynet/20210409-004532/labels.txt',
...
parser.add_argument('--data_dir', type=str, default='data_prepare/Classification/temp',
...
def get_dataset(paths):
    ...
    for i in range(classes_num):
        ...
        imgdir = os.path.join(paths, class_name)
...
def validate(sess, image_list, label_list, enqueue_op, image_paths_placeholder, phase_train_placeholder,
             labels_placeholder,
             batch_size_placeholder, global_step, path_batch, label_batch, logits, predicts, accuracy_top1_batch,
             accuracy_top5_batch,
             cross_entropy_mean,
    ...
    while batch_number < args.epoch_size:
        ...
        step,path_batch_in, label_batch_in, logits_in, predicts_in, accuracy_top1_batch_in, accuracy_top5_batch_in,total_loss_in, cross_loss_in, reg_loss_in \
            = sess.run(
            [global_step, path_batch, label_batch, logits, predicts, accuracy_top1_batch, accuracy_top5_batch,total_loss,
             cross_entropy_mean,
    ...
    f.write("top1 accuracy:" + np.mean(all_top1_acu) + '\n' + "top5 accuracy:" + np.mean(all_top5_acu))
    f.close()
...
def main():
    ...
    if args.mode != "predict":
        ...
        image_list = [os.path.join(args.data_dir, img) for img in images]

    if args.epoch_size is None:
        args.epoch_size = int(math.ceil(len(image_list) / args.batch_size))
    ...
    with sv.managed_session() as sess:
        ...
        if args.mode == "train":
            ...
            while epoch <= max_epochs:
                # Train for one epoch
                step = train(sess, epoch, max_epochs, image_list, label_list, index_dequeue_op, enqueue_op,
            ...
            validate(sess, image_list, label_list, enqueue_op, image_paths_placeholder, phase_train_placeholder,
                     labels_placeholder,
                     batch_size_placeholder, global_step,path_batch, label_batch, logits, predicts, accuracy_top1_batch,
