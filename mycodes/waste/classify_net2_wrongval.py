...
parser.add_argument('--mode', type=str, choices=['train', 'validate', 'predict'], default='validate',
...
parser.add_argument("--supervisor", default="classifynet/20210415-172744",
                    help="directory with checkpoint to resume training from or use for testing")
parser.add_argument("--checkpoint", default="classifynet/20210415-172744",
...
parser.add_argument('--data_dir', type=str, default='data_prepare_20210414/Classification/val',
...
def train(sess, epoch, max_epochs, image_path_list, label_list, index_dequeue_op, enqueue_op, image_paths_placeholder,
          labels_placeholder,
    ...
    image_epoch = np.array(image_path_list)[index_epoch]
    ...
    while batch_number < args.epoch_size:
        ...
        if ((batch_number % 100 == 0) | batch_number == args.epoch_size - 1):
            ...
            total_loss, _, reg_loss, summary_str = sess.run(
                [loss, train_op, regularization_losses, summary_op], feed_dict=feed_dict)
            step = sess.run(global_step)
            summary_writer.add_summary(summary_str, global_step=step)
        else:
            total_loss, _, reg_loss = sess.run([loss, train_op, regularization_losses],
                                               feed_dict=feed_dict)
            step = sess.run(global_step)
        ...
        print('Epoch: [%d][%d/%d]\tTime %.3f\tTotal_Loss %2.3f\tRegLoss %2.3f\timage/sec %0.1f\tremaining %dm' %
              (epoch, batch_number + 1, args.epoch_size, duration, total_loss, np.sum(reg_loss), rate, remaining / 60))
        f = open(os.path.join(output_dir, 'printRecord.txt'), 'a')
        f.write('Epoch: [%d][%d/%d]\tTime %.3f\tTotal_Loss %2.3f\tRegLoss %2.3f\timage/sec %0.1f\tremaining %dm\n' %
                (
                    epoch, batch_number + 1, args.epoch_size, duration, total_loss, np.sum(reg_loss), rate,
                    remaining / 60))
        f.close()
        batch_number += 1
        train_time += duration
...
def validate(sess, image_list, label_list, enqueue_op, image_paths_placeholder, phase_train_placeholder,
             labels_placeholder,
    ...
    all_top5_acu = []
    while batch_number < args.epoch_size:
        path_batch_in, label_batch_in, total_loss_in, logits_in, predicts_in, accuracy_top1_batch_in, accuracy_top5_batch_in, cross_loss_in, reg_loss_in \
            = sess.run(
            [path_batch, label_batch, total_loss, logits, predicts, accuracy_top1_batch, accuracy_top5_batch,
             cross_entropy_mean,
...
def main():
    ...
    global alllabels
    if args.mode == "train":
        alllabels = os.listdir(args.data_dir)
        alllabels.sort()
        ...
        if args.mode == "train":
            ...
            if not os.path.isdir(
                    os.path.join(output_dir,
                                 "bar_imgs")):  # Create the bar imagin directory if it doesn't exist
                os.makedirs(os.path.join(output_dir, "bar_imgs"))
            validate(sess, image_path_list, label_list, enqueue_op, image_paths_placeholder, phase_train_placeholder,
                     labels_placeholder,
                     batch_size_placeholder, path_batch, label_batch, logits, predicts, accuracy_top1_batch,
