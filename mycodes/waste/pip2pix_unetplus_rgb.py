...
def create_generator(generator_inputs, generator_outputs_channels):
    ...
    with tf.variable_scope("encoder_1,1"):
    ...
    for out_channels in layer_specs:
        with tf.variable_scope("encoder_%d,1" % (len(layers[0]) + 1)):
    ...
    for (out_channels, dropout) in enumerate(layer_specs):
        with tf.variable_scope("decoder_2,%d" % (len(layers[1]) + 1)):
    ...
    for out_channels in layer_specs:
        with tf.variable_scope("decoder_3,%d" % (len(layers[2] + 1))):
    ...
    for out_channels in layer_specs:
        with tf.variable_scope("decoder_4,%d" % (len(layers[3] + 1))):
    ...
    with tf.variable_scope("decoder_5,1"):
